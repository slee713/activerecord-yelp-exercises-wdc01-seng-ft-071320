...
Restaurant.all.each { |restaurant| (rand(3..10)).times {Dish.create(name: Faker::Food.dish, restaurant_id: restaurant.id)}}

tag_names.each{|tag_name| Tag.create(name: tag_name)}

tag_ids = Tag.all.map{|tag| tag.id}

Dish.all.each {|dish| (rand(0..3)).times{DishTag.create(dish_id: dish.id, tag_id: tag_ids.sample)}}
